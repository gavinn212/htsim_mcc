...
/*
 * An MCC source and sink
 * Multi-Cell Cooperation protocol implementation
 */
...
class MCCSrc : public BaseQueue, public TriggerTarget {
    friend class MCCSink;
public:
    MCCSrc(MCCLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, linkspeed_bps rate);

    virtual void connect(Route* routeout, Route* routeback, MCCSink& sink, simtime_picosec startTime);
    void set_dst(uint32_t dst) {_dstaddr = dst;}
    void set_traffic_logger(TrafficLogger* pktlogger);

    void startflow();
    void setRate(linkspeed_bps r) {_bitrate = r;_packet_spacing = (simtime_picosec)((Packet::data_packet_size()+MCCPacket::ACKSIZE) * (pow(10.0,12.0) * 8) / _bitrate);doNextEvent();}
    ...
    virtual void processAck(const MCCAck& ack);
    virtual void processNack(const MCCNack& nack);
    ...
    //MCC specific parameters (globals)
    static simtime_picosec _T;//Known baseline RTT
    static double _eta;//Target link utilization
    static double _alpha;//MCC cooperation factor
    static double _beta;//MCC smoothing factor
...
private:
    ...
    MCCPacket::seq_t _last_update_seq;
    MCCPacket::seq_t _cwnd, _flightsize, _Wc;
    uint32_t _incStage;
    double _U;
    linkspeed_bps _pacing_rate;

    double measureInFlight(const MCCAck& ack);
    MCCPacket::seq_t computeWind(double U, bool updateWc);

    inline void update_spacing(){_packet_spacing = (simtime_picosec)((Packet::data_packet_size()+MCCPacket::ACKSIZE) * (pow(10.0,12.0) * 8) / _pacing_rate);}
...
class MCCSink : public PacketSink, public DataReceiver {
    friend class MCCSrc;
public:
    MCCSink();
    ...
    MCCAck::seq_t _cumulative_ack; // the packet we have cumulatively acked
    uint32_t _drops;
...
private:
    ...
    // Connectivity
    void connect(MCCSrc& src, Route* route);
    ...
    MCCPacket::seq_t _last_packet_seqno; //sequence number of the last
    //packet in the connection (or 0 if not known)
    uint64_t _total_received;
    MCCPacket::seq_t _highest_seqno;
 
    // Mechanism
    void send_ack(simtime_picosec ts, IntEntry* intinfo, uint32_t hops);
    void send_nack(simtime_picosec ts, MCCPacket::seq_t ackno);
};
