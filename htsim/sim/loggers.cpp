...
#define HPCCPP_IS_ACK 1<<31
#define HPCCPP_IS_NACK 1<<30
#define HPCCPP_IS_HEADER 1<<28
#define HPCCPP_IS_LASTDATA 1<<27

void HPCCPPTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    HPCCPPPacket& p = static_cast<HPCCPPPacket&>(pkt);
    uint32_t val3=0; // ugly hack to store data in a double

    if (p.type() == HPCCPPACK) {
        val3 |= HPCCPP_IS_ACK;
    } else if (p.type() == HPCCPPNACK) {
        val3 |= HPCCPP_IS_NACK;
    } else if (p.type() == HPCCPP && p.last_packet()) {
        val3 |= HPCCPP_IS_LASTDATA;
    }

    if (p.header_only())
        val3 |= HPCCPP_IS_HEADER;
    
    _logfile->writeRecord(Logger::HPCCPP_TRAFFIC,
                          location.get_id(),
                          ev,
                          p.flow().get_id(),
                          p.id(),
...
string HPCCPPTrafficLogger::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    assert(event._type == Logger::HPCCPP_TRAFFIC);
    ss << " Type HPCCPPTRAFFIC ID " << event._id;
    switch((TrafficLogger::TrafficEvent)event._ev) {
    case PKT_ARRIVE:
        ss << " Ev ARRIVE ";
        break;
    ...
    ss << " FlowID " << (uint64_t)event._val1;
    uint32_t val3i = (uint32_t)event._val3;
    if (val3i & HPCCPP_IS_ACK) { 
        ss << " Ptype ACK"
           << " Ackno " << (uint64_t)event._val2;
    } else if (val3i & HPCCPP_IS_NACK) {
        ss << " Ptype NACK"
           << " Ackno " << (uint64_t)event._val2;
    } else {
        ss << " Ptype DATA"
           << " Seqno " << (uint64_t)event._val2;
        if (val3i & HPCCPP_IS_LASTDATA) {
            ss << " flag LASTDATA";
        }
    }
    if (val3i & HPCCPP_IS_HEADER) 
        ss << " Psize HEADER";
    else
        ss << " Psize FULL";
    return ss.str();
...
HPCCPPSinkLoggerSampling::HPCCPPSinkLoggerSampling(simtime_picosec period, 
                                               EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::HPCCPP_SINK, HPCCPPLogger::RATE)
{
    cout << "HPCCPPSinkLoggerSampling(p=" << timeAsSec(period) << " init \n";
}

void HPCCPPSinkLoggerSampling::doNextEvent(){
    eventlist().sourceIsPendingRel(*this,_period);  
    simtime_picosec now = eventlist().now();
    simtime_picosec delta = now - _last_time;
    _last_time = now;
    ...
    for (uint64_t i = 0; i<_sinks.size(); i++){
        HPCCPPSink *sink = (HPCCPPSink*)_sinks[i];
        if (_last_seq[i] <= sink->total_received()) {
            deltaB = sink->total_received() - _last_seq[i];
            if (delta > 0)
                rate = deltaB * 1000000000000.0 / delta;//Bps
...
string HPCCPPSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
    case Logger::HPCCPP_SINK:
        assert(event._ev == HPCCPPLogger::RATE);
        ss << " Type HPCCPP_SINK ID " << event._id << " Ev RATE"
           << " CAck " << (uint64_t)event._val1 << " Rate " << (uint64_t)event._val3;
        break;
    default:
        ss << "Unknown event " << event._type;
...
#define MCC_IS_ACK 1<<31
#define MCC_IS_NACK 1<<30
#define MCC_IS_HEADER 1<<28
#define MCC_IS_LASTDATA 1<<27

void MCCTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    MCCPacket& p = static_cast<MCCPacket&>(pkt);
    uint32_t val3=0; // ugly hack to store data in a double

    if (p.type() == MCCACK) {
        val3 |= MCC_IS_ACK;
    } else if (p.type() == MCCNACK) {
        val3 |= MCC_IS_NACK;
    } else if (p.type() == MCC && p.last_packet()) {
        val3 |= MCC_IS_LASTDATA;
    }

    if (p.header_only())
        val3 |= MCC_IS_HEADER;
    
    _logfile->writeRecord(Logger::MCC_TRAFFIC,
                          location.get_id(),
                          ev,
                          p.flow().get_id(),
                          p.id(),
...
string MCCTrafficLogger::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    assert(event._type == Logger::MCC_TRAFFIC);
    ss << " Type MCCTRAFFIC ID " << event._id;
    switch((TrafficLogger::TrafficEvent)event._ev) {
    case PKT_ARRIVE:
        ss << " Ev ARRIVE ";
        break;
    ...
    case PKT_TRIM:
    case PKT_BOUNCE:
        abort(); // shouldn't happen with MCC
    }
    ss << " FlowID " << (uint64_t)event._val1;
    uint32_t val3i = (uint32_t)event._val3;
    if (val3i & MCC_IS_ACK) { 
        ss << " Ptype ACK"
           << " Ackno " << (uint64_t)event._val2;
    } else if (val3i & MCC_IS_NACK) {
        ss << " Ptype NACK"
           << " Ackno " << (uint64_t)event._val2;
    } else {
        ss << " Ptype DATA"
           << " Seqno " << (uint64_t)event._val2;
        if (val3i & MCC_IS_LASTDATA) {
            ss << " flag LASTDATA";
        }
    }
    if (val3i & MCC_IS_HEADER) 
        ss << " Psize HEADER";
    else
        ss << " Psize FULL";
    return ss.str();
...
MCCSinkLoggerSampling::MCCSinkLoggerSampling(simtime_picosec period, 
                                               EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::MCC_SINK, MCCLogger::RATE)
{
    cout << "MCCSinkLoggerSampling(p=" << timeAsSec(period) << " init \n";
}

void MCCSinkLoggerSampling::doNextEvent(){
    eventlist().sourceIsPendingRel(*this,_period);  
    simtime_picosec now = eventlist().now();
    simtime_picosec delta = now - _last_time;
    _last_time = now;
...
string MCCSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
    case Logger::MCC_SINK:
        assert(event._ev == MCCLogger::RATE);
        ss << " Type MCC_SINK ID " << event._id << " Ev RATE"
           << " CAck " << (uint64_t)event._val1 << " Rate " << (uint64_t)event._val3;
        break;
    default:
        ss << "Unknown event " << event._type;
