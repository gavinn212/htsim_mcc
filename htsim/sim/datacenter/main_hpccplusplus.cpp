...
int main(int argc, char **argv) {
    ...
    double hpccpp_beta = HPCCPPSrc::_beta;
    double hpccpp_gamma = HPCCPPSrc::_gamma;
    uint32_t hpccpp_min_cwnd_pkts = HPCCPPSrc::_min_cwnd_pkts;
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")) {
            ...
            i+=2;
        } else if (!strcmp(argv[i],"-hpccpp_beta")){
            hpccpp_beta = atof(argv[i+1]);
            cout << "HPCC++ beta (INT smoothing) " << hpccpp_beta << endl;
            i++;
        } else if (!strcmp(argv[i],"-hpccpp_gamma")){
            hpccpp_gamma = atof(argv[i+1]);
            cout << "HPCC++ gamma (cwnd smoothing) " << hpccpp_gamma << endl;
            i++;
        } else if (!strcmp(argv[i],"-hpccpp_mincwnd_pkts")){
            hpccpp_min_cwnd_pkts = atoi(argv[i+1]);
            cout << "HPCC++ minimum cwnd (pkts) " << hpccpp_min_cwnd_pkts << endl;
            i++;
        } else if (!strcmp(argv[i],"-debug")){
            HPCCPPSrc::_debug = true;
    ...
    HPCCPPSrc::_beta = hpccpp_beta;
    HPCCPPSrc::_gamma = hpccpp_gamma;
    HPCCPPSrc::_min_cwnd_pkts = hpccpp_min_cwnd_pkts;
    ...
    HPCCPPSinkLoggerSampling sinkLogger = HPCCPPSinkLoggerSampling(timeFromMs(logtime), eventlist);
    if (log_sink) {
        logfile.addLogger(sinkLogger);
    }
    HPCCPPTrafficLogger traffic_logger = HPCCPPTrafficLogger();
    if (log_traffic) {
        logfile.addLogger(traffic_logger);
    }

    HPCCPPSrc* hpccppSrc;
    HPCCPPSink* hpccppSnk;
    ...
    for (size_t c = 0; c < all_conns->size(); c++){
        ...
        hpccppSrc = new HPCCPPSrc(NULL, NULL, eventlist,linkspeed);

        hpccpp_srcs.push_back(hpccppSrc);
        hpccppSrc->set_dst(dest);
                        
        if (crt->size>0){
            hpccppSrc->set_flowsize(crt->size);
        }

        if (crt->flowid) {
            hpccppSrc->set_flowid(crt->flowid);
            assert(flowmap.find(crt->flowid) == flowmap.end()); // don't have dups
            flowmap[crt->flowid] = hpccppSrc;
        }

        if (crt->trigger) {
            Trigger* trig = conns->getTrigger(crt->trigger, eventlist);
            trig->add_target(*hpccppSrc);
        }
        if (crt->send_done_trigger) {
            Trigger* trig = conns->getTrigger(crt->send_done_trigger, eventlist);
            hpccppSrc->set_end_trigger(*trig);
        }

        hpccppSnk = new HPCCPPSink();
                        
        hpccppSrc->setName("HPCCPP_" + ntoa(src) + "_" + ntoa(dest));

        logfile.writeName(*hpccppSrc);

        hpccppSnk->set_src(src);
                        
        hpccppSnk->setName("HPCCPP_sink_" + ntoa(src) + "_" + ntoa(dest));
        logfile.writeName(*hpccppSnk);
                        
        ((HostQueue*)top->queues_ns_nlp[src][topo_cfg->HOST_POD_SWITCH(src)][0])->addHostSender(hpccppSrc);
        ...
        if (route_strategy!=SINGLE_PATH && route_strategy!=ECMP_FIB){
            ...
            hpccppSrc->connect(srctotor, dsttotor, *hpccppSnk, crt->start);

            //register src and snk to receive packets from their respective TORs. 
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(src,hpccppSrc->flow_id(),hpccppSrc);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(dest,hpccppSrc->flow_id(),hpccppSnk);
        } else {
            int choice = rand()%net_paths[src][dest]->size();
            routeout = new Route(*(net_paths[src][dest]->at(choice)));
            routeout->add_endpoints(hpccppSrc, hpccppSnk);
                                
            routein = new Route(*top->get_bidir_paths(dest,src,false)->at(choice));
            routein->add_endpoints(hpccppSnk, hpccppSrc);
            hpccppSrc->connect(routeout, routein, *hpccppSnk, timeFromUs((uint32_t)rand()%20));
        }
        ...
        if (log_sink) {
            sinkLogger.monitorSink(hpccppSnk);
        }
    }
    ...
    for (size_t ix = 0; ix < hpccpp_srcs.size(); ix++) {
        new_pkts += hpccpp_srcs[ix]->_new_packets_sent;
        rtx_pkts += hpccpp_srcs[ix]->_rtx_packets_sent;
    }
