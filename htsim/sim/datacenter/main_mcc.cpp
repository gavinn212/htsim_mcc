...
int main(int argc, char **argv) {
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")) {
            ...
            i+=2;
        } else if (!strcmp(argv[i],"-mcc_alpha")){
            mcc_alpha = atof(argv[i+1]);
            cout << "MCC alpha (cooperation factor) set to " << mcc_alpha << endl;
            i++;
        } else if (!strcmp(argv[i],"-mcc_beta")){
            mcc_beta = atof(argv[i+1]);
            cout << "MCC beta (smoothing factor) set to " << mcc_beta << endl;
            i++;
    ...
    // Set MCC parameters
    MCCSrc::_alpha = mcc_alpha;
    MCCSrc::_beta = mcc_beta;
    ...
    // prepare the loggers
    // Note: For now, we'll reuse HPCC loggers since MCC structure is similar
    // In a full implementation, you'd want MCC-specific loggers
    ...
    MCCSinkLoggerSampling sinkLogger = MCCSinkLoggerSampling(timeFromMs(logtime), eventlist);
    if (log_sink) {
        logfile.addLogger(sinkLogger);
    }
    MCCTrafficLogger traffic_logger = MCCTrafficLogger();
    if (log_traffic) {
    ...
    for (size_t c = 0; c < all_conns->size(); c++){
        ...
        mccSrc = new MCCSrc(NULL, NULL, eventlist,linkspeed);

        mcc_srcs.push_back(mccSrc);
        mccSrc->set_dst(dest);
                        
        if (crt->size>0){
            mccSrc->set_flowsize(crt->size);
        }

        if (crt->flowid) {
            mccSrc->set_flowid(crt->flowid);
            assert(flowmap.find(crt->flowid) == flowmap.end()); // don't have dups
            flowmap[crt->flowid] = mccSrc;
        }

        if (crt->trigger) {
            Trigger* trig = conns->getTrigger(crt->trigger, eventlist);
            trig->add_target(*mccSrc);
        }
        if (crt->send_done_trigger) {
            Trigger* trig = conns->getTrigger(crt->send_done_trigger, eventlist);
            mccSrc->set_end_trigger(*trig);
        }

        if (log_traffic) {
            mccSrc->set_traffic_logger(&traffic_logger);
        }

        mccSnk = new MCCSink();
                        
        mccSrc->setName("MCC_" + ntoa(src) + "_" + ntoa(dest));

        logfile.writeName(*mccSrc);

        mccSnk->set_src(src);
                        
        mccSnk->setName("MCC_sink_" + ntoa(src) + "_" + ntoa(dest));
        logfile.writeName(*mccSnk);
                        
        ((HostQueue*)top->queues_ns_nlp[src][topo_cfg->HOST_POD_SWITCH(src)][0])->addHostSender(mccSrc);
        ...
        if (route_strategy!=SINGLE_PATH && route_strategy!=ECMP_FIB){
            ...
            mccSrc->connect(srctotor, dsttotor, *mccSnk, crt->start);

            //register src and snk to receive packets from their respective TORs. 
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            assert(top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(src)]->addHostPort(src,mccSrc->flow_id(),mccSrc);
            top->switches_lp[topo_cfg->HOST_POD_SWITCH(dest)]->addHostPort(dest,mccSrc->flow_id(),mccSnk);
        } else {
            int choice = rand()%net_paths[src][dest]->size();
            routeout = new Route(*(net_paths[src][dest]->at(choice)));
            routeout->add_endpoints(mccSrc, mccSnk);
                                
            routein = new Route(*top->get_bidir_paths(dest,src,false)->at(choice));
            routein->add_endpoints(mccSnk, mccSrc);
            mccSrc->connect(routeout, routein, *mccSnk, timeFromUs((uint32_t)rand()%20));
        }
    ...
    logfile.write("# mcc_alpha=" + ntoa(mcc_alpha));
    logfile.write("# mcc_beta=" + ntoa(mcc_beta));
    ...
    for (size_t ix = 0; ix < mcc_srcs.size(); ix++) {
        new_pkts += mcc_srcs[ix]->_new_packets_sent;
        rtx_pkts += mcc_srcs[ix]->_rtx_packets_sent;
    }
