...
/* When you're debugging, sometimes it's useful to enable debugging on
   a single MCC receiver, rather than on all of them.  Set this to the
...
/* keep track of RTOs.  Generally, we shouldn't see RTOs if
   ...
   large incasts. */
uint32_t MCCSrc::_global_node_count = 0;

simtime_picosec MCCSrc::_T = timeFromUs(12.0);//Known baseline RTT
double MCCSrc::_eta = 0.95;//Target link utilization
double MCCSrc::_alpha = 0.5;//MCC cooperation factor
double MCCSrc::_beta = 0.3;//MCC smoothing factor
uint32_t MCCSrc::_max_stages = 5;//Maximum stages for additive increases
uint32_t MCCSrc::_N = 10;//maximum number of flows.
uint32_t MCCSrc::_Wai = 0;//Additive increase amount.

MCCSrc::MCCSrc(MCCLogger* logger, TrafficLogger* pktlogger, EventList &eventlist, linkspeed_bps rate)
    : BaseQueue(rate,eventlist,NULL), _logger(logger), _flow(pktlogger)
{
    ...
    _nodename = "MCCsrc " + to_string(_node_num);
...
void MCCSrc::set_traffic_logger(TrafficLogger* pktlogger) {
...
void MCCSrc::log_me() {
    ...
    cout << "Enabling logging on MCCSrc " << _nodename << endl;
    _log_me = true;
    if (_sink)
        _sink->log_me();
}

void MCCSrc::startflow(){
...
void MCCSrc::set_end_trigger(Trigger& end_trigger) {
    _end_trigger = &end_trigger;
}

void MCCSrc::connect(Route* routeout, Route* routeback, MCCSink& sink, simtime_picosec starttime) {
    assert(routeout);
    _route = routeout;
    
    _sink = &sink;
    _flow.set_id(get_id()); // identify the packet flow with the MCC source that generated it
    _flow._name = _name;
...
/* Process a NACK.  Generally this involves queuing the NACKed packet
   ...
   resend immediately */
void MCCSrc::processNack(const MCCNack& nack){
    _last_acked = nack.ackno();
    _rtx_packets_sent += _highest_sent - _last_acked;

    cout << "MCC " << _name << " go back n from " <<  _highest_sent << " to " << _last_acked << " at " << timeAsUs(eventlist().now()) << " us" << endl;
...
void MCCSrc::processAck(const MCCAck& ack) {
    MCCAck::seq_t ackno = ack.ackno();
    ...
    //MCC magic here - Multi-Cell Cooperation algorithm
    /*
      MCC algorithm:
      - Similar to HPCC but with multi-cell cooperation factors
      - Uses alpha for cooperation and beta for smoothing
    */   
    ...
    if (_logger) _logger->logMCC(*this, MCCLogger::MCC_RCV);
...
void MCCSrc::processPause(const EthPausePacket& p) {
    if (p.sleepTime()>0){
...
void MCCSrc::receivePacket(Packet& pkt) 
{
    if (!_flow_started){
    ...
    case MCCNACK: 
        _nacks_received++;
        processNack((const MCCNack&)pkt);
        pkt.free();
        return;
    case MCCACK:
        _acks_received++;
        processAck((const MCCAck&)pkt);
        pkt.free();
        return;
    default:
        abort();
...
void MCCSrc::send_packet() {
    ...
    p = MCCPacket::newpkt(_flow, *_route, _highest_sent+1, _mss, false, last_packet,_dstaddr);
...
double MCCSrc::measureInFlight(const MCCAck& ack){
    /*    MCC version of measureInFlight
          Similar to HPCC but with MCC-specific smoothing using beta factor
          2:  u = 0;
          ...
          8:     tau = min(tau, T);
          9:     U = (1 - beta)*U + beta*u;  // MCC uses beta for smoothing
          10:    return U;*/
    ...
    if (ack._int_hop == _link_count){
        ...
        // MCC uses beta for smoothing instead of tau/T
...
MCCPacket::seq_t MCCSrc::computeWind(double U, bool updateWc){
    /*
      MCC version of ComputeWind
      Uses alpha for cooperation factor
      11: Function ComputeWind(U, updateWc)
      12:    if U >= eta or incStage >= maxStagee then
      13:             Wc
      W = ----- + W_ai * (1 + alpha);  // MCC adds cooperation factor
      U/eta
      14:        if updateWc then
      15:            incStagee = 0; Wc = W ;
      16:    else
      17:        W = Wc + W_ai * (1 + alpha) ;  // MCC adds cooperation factor
      18:        if updateWc then
      19:            incStage++; Wc = W ;
      20:    return W
    */
...
MCCSink::MCCSink()
    : DataReceiver("MCC_sink"),_cumulative_ack(0) , _total_received(0) 
{
    _src = 0;
...
void MCCSink::connect(MCCSrc& src, Route* route)
{
    _src = &src;
    _route = route;
...
void MCCSink::receivePacket(Packet& pkt) {
    switch (pkt.type()) {
    case MCC:
        break;
    default:
    ...
    MCCPacket *p = (MCCPacket*)(&pkt);
    MCCPacket::seq_t seqno = p->seqno();
    ...
    if (seqno > _cumulative_ack+1){
        ...
        //cout << "Wrong seqno received at MCC SINK " << seqno << " expecting " << _cumulative_ack << endl;
        return;
    }

    int size = p->size()-MCCPacket::ACKSIZE; 
...
void MCCSink::send_ack(simtime_picosec ts, IntEntry* intinfo, uint32_t hops) {
    MCCAck *ack = 0;
    ack = MCCAck::newpkt(_src->_flow, *_route, _cumulative_ack,_srcaddr);
    ack->set_pathid(0);
...
void MCCSink::send_nack(simtime_picosec ts, MCCPacket::seq_t ackno) {
    MCCNack *nack = NULL;
    nack = MCCNack::newpkt(_src->_flow, *_route, ackno,_srcaddr);
